...
if ( ! class_exists( 'Astra_Ext_Advanced_Hooks_Markup' ) ) {
	...
	class Astra_Ext_Advanced_Hooks_Markup {
		 ...
		 * Admin Menu
		 *
		 * @param  object $wp_admin_bar Menu object.
		 * @return void
		 */
		function admin_bar_menu( $wp_admin_bar ) {
			...
			if ( class_exists( 'Brizy_Editor_Post' ) ) {
				$post = self::is_brizy_activated( get_the_id() );

				if ( $post && $post->uses_editor() ) {
					$type = $post->get_wp_post()->post_type;
					$args = array(
						'id'    => 'brizy_Edit_page_link',
						/* translators: %s is the page title */
						'title' => sprintf( __( 'Edit %s with Brizy', 'astra-addon' ), get_the_title( get_the_id() ) ),
						'href'  => $post->edit_url(),
						'meta'  => array(),
					);
		...
		public function load_markup() {
			...
			foreach ( $result as $post_id => $post_data ) {
				...
				/**
				 * Load the scripts of all the hooks.
				 *
				 * @todo Use method `initialize_front_end()` of class `Brizy_Public_Main` to initialize the front end.
				 */
				$post = self::is_brizy_activated( $post_id );
				...
				if ( $post && $post->uses_editor() ) {
					...
					add_action(
						'admin_bar_menu', function( $wp_admin_bar ) use ( $post ) {
		 ...
		 * @return boolean
		 */
		function is_brizy_activated( $post_id ) {
			if ( class_exists( 'Brizy_Editor_Post' ) ) {

				$supported_post_types = brizy()->supported_post_types();

				if ( in_array( ASTRA_ADVANCED_HOOKS_POST_TYPE, $supported_post_types ) ) {
